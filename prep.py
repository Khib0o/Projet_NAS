...
def DHCP(inp):
    ...
    for L in Ls.values():
        subnet = L.name[1:]
        c=1
        L.network = NET_PREF+f"{subnet}.0"
        # print(L.name,L.network)
        for term in [L.term1,L.term2]:
            ip=L.network[:-1]+str(c)
            Rs[term.Router_id].add_ints(L.name,term.inter,ip,24)
            c=c+1
            # print(Rs[term.Router_id].name,term.inter,ip)
        ...
        if rout.name in CEs():
            ...
            for l in links:
                if rout.name==l.term1.Router_name:
                    ...
                    b["CEp"][rout.name]["neighbor"].append(([Rs[l.term2.Router_id].intfs[l.term2.inter].ip,1],l.term2.Router_name))
                if rout.name==l.term2.Router_name:
                    b["CE"][rout.name]["neighbor"].append(([rout.intfs[l.term2.inter].ip,c],l.term1.Router_name))
                    b["CEp"][rout.name]["neighbor"].append(([Rs[l.term1.Router_id].intfs[l.term1.inter].ip,1],l.term1.Router_name))
            c=c+1
    # print(b)
    return b
            ...
            for v in vpn_ls():
                if r.name in [i[0] for i in v["concerned_PEs"]] :
                    ...
                    for k in bgp()["CE"].values():
                        consL=[i for i in v["concerned_PEs"] if i[0]==r.name][0][1]
                        consIP=[Rs[consL.term1.Router_id].intfs[consL.term1.inter].ip,Rs[consL.term2.Router_id].intfs[consL.term2.inter].ip]
                        # print(consIP)
                        # print(k["neighbor"])
                        # print(k["neighbor"][0][0][0]==v["concerned_PEs"])
                        if k["neighbor"][0][1]==r.name and k["neighbor"][0][0][0] in consIP:
...
# for l in surf(raw)[0].values():
#         couple1=(Rs[l.term1.Router_id].name,l.term1.inter)
#         ip1= Rs[l.term1.Router_id].intfs[l.term1.inter].ip
#         ip2= Rs[l.term2.Router_id].intfs[l.term2.inter].ip
#         couple2=(Rs[l.term2.Router_id].name,l.term2.inter)
        # print(l.network,':\t',couple1,":",ip1,'\t',couple2,':',ip2)
        # print(l)
